...
export default class SrsReviewSession extends React.Component<ISrsReviewSessionProps, ISrsReviewSessionState> {
  ...
  public render(): React.ReactElement<ISrsReviewSessionProps> {    
    ...
    return (
        <section>
          { this.state.sessionReviewItems.length > 0
            ? (
              <div>
                <div>{ reviewItem.Item }</div>
                <div>{ "Answer is: " + reviewItem[answerStateItem] }</div>
                <div>{ itemSessionReviewType }</div>
                <input id="answerBox" type="text" name={ answerStateItem } onKeyPress={ (event) => { 
                  if (event.key == 'Enter') {
